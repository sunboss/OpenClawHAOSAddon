...
# Channel tokens  - dedicated fields for common channels (auto-written to openclaw.json)
TELEGRAM_BOT_TOKEN=$(jq -r '.telegram_bot_token // empty' "$OPTIONS_FILE")
DISCORD_BOT_TOKEN=$(jq -r '.discord_bot_token // empty' "$OPTIONS_FILE")
SLACK_BOT_TOKEN=$(jq -r '.slack_bot_token // empty' "$OPTIONS_FILE")
SLACK_APP_TOKEN=$(jq -r '.slack_app_token // empty' "$OPTIONS_FILE")
...
if ! validate_public_url "$GW_PUBLIC_URL"; then
  echo "WARN: gateway_public_url is not a valid http(s) URL: '$GW_PUBLIC_URL'"
  echo "WARN: The landing page button may be hidden until this is fixed."
fi
...
# This add-on only bootstraps a minimal config when missing and applies add-on-specific settings.
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Inject channel tokens from dedicated add-on config fields into openclaw.json.
# This avoids requiring users to hand-edit JSON for the most common channels.
# Tokens are written only when the field is non-empty; existing values are
# preserved if the field is left blank.
# ------------------------------------------------------------------------------
inject_channel_token() {
  local json_path="$1"   # e.g. channels.telegram.botToken
  local token_value="$2"
  local label="$3"

  [ -z "$token_value" ] && return 0

  if python3 "$HELPER_PATH" set-json-path "$json_path" "$token_value" 2>/dev/null; then
    echo "INFO: Injected $label into openclaw.json ($json_path)"
  else
    # Fallback: use openclaw config set if helper doesn't support set-json-path
    openclaw config set "$json_path" "$token_value" 2>/dev/null && \
      echo "INFO: Injected $label via openclaw config set" || \
      echo "WARN: Could not inject $label  - set it manually via the terminal"
  fi
}

if [ -f "$OPENCLAW_CONFIG_PATH" ] && [ -f "$HELPER_PATH" ]; then
  inject_channel_token "channels.telegram.botToken" "$TELEGRAM_BOT_TOKEN" "Telegram bot token"
...
# Apply sandbox mode to openclaw.json
if [ -f "$OPENCLAW_CONFIG_PATH" ]; then
  if [ "$SANDBOX_MODE" = "non-main" ]; then
    python3 - <<PY
import json
from pathlib import Path
p = Path('$OPENCLAW_CONFIG_PATH')
...
cfg = json.loads(p.read_text())
try:
    del cfg['agents']['defaults']['sandbox']['mode']
    p.write_text(json.dumps(cfg, indent=2) + '\n')
    print('INFO: sandbox_mode=off: removed sandbox.mode from openclaw.json')
...
if [ "$ENABLE_HTTPS_PROXY" = "true" ]; then
  ...
  if [ ! -f "$CERT_DIR/ca.key" ] || [ ! -f "$CERT_DIR/ca.crt" ]; then
    ...
    if ! openssl genrsa -out "$CERT_DIR/ca.key" 2048 2>/dev/null; then
      echo "ERROR: Failed to generate CA private key."
      exit 1
    fi
    if ! openssl req -new -x509 -key "$CERT_DIR/ca.key" -out "$CERT_DIR/ca.crt" \
      -days 3650 -nodes -subj "/CN=OpenClaw Local CA" 2>/dev/null; then
...
PY
  ...
  if [ ! -f "$CERT_DIR/gateway.crt" ] || [ ! -f "$CERT_DIR/gateway.key" ] || [ "$LAN_IP" != "$STORED_IP" ] || [ "$EXTRA_SANS" != "$STORED_EXTRA_SANS" ]; then
    ...
    if ! openssl genrsa -out "$CERT_DIR/gateway.key" 2048 2>/dev/null; then
      echo "ERROR: Failed to generate gateway private key."
      exit 1
    fi
    if ! openssl req -new -key "$CERT_DIR/gateway.key" -out "$CERT_DIR/gateway.csr" \
      -subj "/CN=OpenClaw Gateway" 2>/dev/null; then
...
PY
  ...
  # 娑擃厽鏋冪拠瀛樻閿涙碍鏌婄憗鍛繁閸掕泛鐣ㄩ崗銊х摜閻ｃ儻绱欐稉宥呬粵閸忕厧顔愰梽宥囬獓閿涘绱?  # - dangerouslyDisableDeviceAuth=false閿涘牆鎯庨悽銊啎婢跺洩顓荤拠渚婄礆
...
# without blocking startup. Output goes to add-on logs.
(
  sleep 15  # give gateway time to initialise
  if command -v openclaw >/dev/null 2>&1; then
...
echo "Starting local action server on 127.0.0.1:48100 ..."
python3 /action_server.py &
ACTION_PID=$!
sleep 1
if kill -0 "$ACTION_PID" 2>/dev/null; then
  echo "action server started with PID $ACTION_PID"
else
  echo "WARN: action server failed to start; ingress service controls will be unavailable"
fi
...
# 娑擃厽鏋冪拠瀛樻閿涙矮绗夐崘宥嗗Ω token 濞夈劌鍙嗘い鐢告桨閹?URL閿涘矂浼╅崗宥呭殶閹诡喗纭犲蹇撳煂閸樺棗褰剁拋鏉跨秿/閺冦儱绻旈妴?
...
# Collect memory usage for landing page
...
if [ -f /proc/meminfo ]; then
  ...
  if [ -n "$MEM_TOTAL_KB" ] && [ -n "$MEM_AVAIL_KB" ]; then
    ...
    MEM_TOTAL="${MEM_TOTAL_KB}K"
    MEM_USED="${MEM_USED_KB}K"
    MEM_PCT=$(( MEM_USED_KB * 100 / MEM_TOTAL_KB ))
    # Convert to MB for display
    MEM_TOTAL="$(( MEM_TOTAL_KB / 1024 ))M"
    MEM_USED="$(( MEM_USED_KB / 1024 ))M"
    MEM_PCT="${MEM_PCT}"
  fi
fi

# Collect CPU load average (1-minute) as rough CPU % proxy
CPU_PCT="0"
if [ -f /proc/loadavg ]; then
  LOAD1=$(awk '{print $1}' /proc/loadavg)
  NCPU=$(nproc 2>/dev/null || echo 1)
  # Express as percentage of total CPU capacity, cap at 100
  CPU_PCT=$(python3 -c "v=round(float('${LOAD1}')/int('${NCPU}')*100,1);print(min(v,100))" 2>/dev/null || echo "0")
fi

mkdir -p /etc/nginx/html
GATEWAY_TOKEN_FILE="/etc/nginx/html/gateway.token"
GATEWAY_TOKEN_VALUE=""
if [ -f "$OPENCLAW_CONFIG_PATH" ]; then
  GATEWAY_TOKEN_VALUE="$(jq -r '.gateway.auth.token // empty' "$OPENCLAW_CONFIG_PATH" 2>/dev/null || echo "")"
fi
printf '%s' "$GATEWAY_TOKEN_VALUE" > "$GATEWAY_TOKEN_FILE"
chmod 644 "$GATEWAY_TOKEN_FILE" 2>/dev/null || true

GW_PUBLIC_URL="$GW_PUBLIC_URL" TERMINAL_PORT="$TERMINAL_PORT" \
